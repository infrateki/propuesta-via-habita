...
| Apr 29 | Proposal content updates per Pablo's round-2 feedback | Serge | **6 profiles** (Admin OT · Gerencia · Desarrollo/Proyectistas · Calidad · Supervisor/Jefe terreno · Trazador/Operario) — `data/proposal.ts` MODULES m9, `data/comparison-scenarios.ts` exclusives card, all 3 markdowns (PROPOSAL-v4, v5, public/proposal.md) §10 + §12.3 tables. **Capacitación target +10** (was ≥4) — `data/proposal.ts` KPIS capacitados + GATE_CRITERIA g4, all 3 markdowns §3.1 row E + §7 mes 3 + §8.1 KPI row + §8.3 gate row. **Trazador in problem section** — `data/proposal.ts` PROBLEMS p3 reframed around the trazador as the canonical reach-failure user (1 de ~100 stakeholders). **NEW Vision section** — `components/proposal/VisionSection.tsx` (4 horizons: cartera completa, ciclo de vida, postventa digital, entregas oficiales DOM/bomberos) wired into `/propuesta` after GateSection at §09; closer bumped to §10. Markdown §18 inserted as "Horizonte estratégico", current §18 shifted to §19, §19 to §20. **Próximos pasos rewritten** with Pablo's confirmed dates (4–8 mayo working sessions → 8 mayo Héctor regresa → 11 mayo sync con nuevo lead OT → 12–13 mayo presentación dirección → 15 mayo cierre objetivo → 1 junio kickoff) + notes on Marcela's transition to project coordinator and her OT replacement starting Monday. |
| Apr 29 | v9.0 cut + site version alignment | Serge | NEW: `PROPOSAL-v9.md` (header changelog block lists all v9 changes) · `public/proposal.md` synced to v9 · `public/proposal.pdf` regenerated (621 KB, **INFRATEK logo embedded** in cover header via base64) · `INFRATEK_Propuesta_VIA-HABITA_2026_v9.pdf` + `INFRATEK_Propuesta_VIA-HABITA_2026_v9.docx` at repo root. Render script enhanced: `scripts/render-proposal.mjs` now reads `app/icon.png` and writes a cover header above the markdown body; default source bumped to PROPOSAL-v9.md. **Site version refs aligned to v9.0 · 29 ABR 2026**: `app/page.tsx`, `app/propuesta/page.tsx`, `app/documento/page.tsx` (×4 strings), `components/layout/Footer.tsx`. **Download filenames bumped to `_v9.pdf`**: `components/contact/DownloadProposal.tsx`, `components/layout/Navigation.tsx` (×2), `components/configurator/PriceSummary.tsx`, `app/propuesta/page.tsx` hero CTA. Comments in `data/proposal.ts` and `lib/constants.ts` reference v9.0. ⚠ No signature image found in repo — markdown signature blocks (`Firma · Fecha`) render as before. Earlier v5/v6 PDFs and PROPOSAL-v5.md retained as historical record. |
| Apr 29 | Reconciliation pass: restored multi-platform Precios tab and Autodesk costs alongside Pablo's 3-scenario narrative | Serge | **4-tab comparativo: Análisis ‖ Precios ‖ Costos Autodesk ‖ Funcionalidades.** `app/comparativo/page.tsx` rewritten with 4 panels: `AnalisisPanel` (`ComparativoNarrative` — Pablo's 5-section narrative, default tab), `PreciosPanel` (`ComparisonTable` + `CostChart` + 50-user benchmark callout — restored from earlier orphan), `AutodeskPanel` (`AutodeskCosts` — full AEC catalog + Habita scenario, restored), `FuncionalidadesPanel` (`FeatureMatrix`). Hero copy reframed to introduce the four lenses. **VisionSection trimmed**: replaced 4 large horizon cards + separate quote callout + acotación aside with a tighter 2-col layout (quote anchor + numbered horizon list) — same content, ~40% less vertical space, reads as a closing note. Eyebrow shortened to "Horizonte". Lede absorbed the acotación. **Home page**: replaced the "// sección en construcción · contenido extraído de v4.0 en Phase 3" placeholder paragraph (visible to users) with a real teaser describing the diagnostic + trazador hook and a link to `/propuesta#problema`. ⚠ No data files modified — `data/platforms.ts` (12 platforms with 50-user benchmark) and `data/autodesk-costs.ts` (full AEC catalog) were already intact from earlier; no restoration needed. |
| Apr 29 | v10.0 cut sobre la base correcta (v8.pdf, no v5.md) | Serge | El v9 cut anterior fue armado encima de PROPOSAL-v5.md, pero el documento canónico que Sergio venía iterando era `INFRATEK_Propuesta_VIA-HABITA_2026_v8.pdf` (10 páginas, formato carta ejecutiva, contiene v6.0 internamente, con equivalencias CLP/UF). Acción: extraje el texto de v8.pdf con un script puntual (`scripts/extract-v8.mjs` usando `pdf-parse` v2.4.5 ESM) y construí **`PROPOSAL-v10.md`** preservando el formato y tono ejecutivo de v8 + sobreponiendo el feedback de Pablo del 29 abril (3-scenario comparativo §13, 6 perfiles §8, §14 Horizonte estratégico, §16 Próximos pasos con working sessions 4–8 mayo / Héctor 8 mayo / sync 11 mayo / presentación dirección 12–13 mayo / cierre 15 mayo / kickoff 1 jun, +10 trazador, hilo trazador en §1 contexto). PDF/DOCX regenerados: `INFRATEK_Propuesta_VIA-HABITA_2026_v10.pdf` (388 KB) y `_v10.docx` (481 KB) en repo root + `public/proposal.pdf` actualizado. **Site refs bumped v9 → v10**: `app/page.tsx`, `app/propuesta/page.tsx`, `app/documento/page.tsx` (×4), `components/layout/Footer.tsx`, `components/contact/DownloadProposal.tsx`, `components/layout/Navigation.tsx` (×2), `components/configurator/PriceSummary.tsx`, comments in `data/proposal.ts` y `lib/constants.ts`. Render scripts: defaults bumped to PROPOSAL-v10.md / `_v10.docx`. PROPOSAL-v9.md retenido como histórico. **pdf-parse v2.4.5** añadido a devDependencies para futuras importaciones. |
| Apr 30 | v11.0: neutralize tone, deploy updated PDF, soften sales language | Serge | **Sin cambios estructurales** — pricing, fechas, 3-scenario comparativo, 6 perfiles, +10, Vision, trazador, Horizonte estratégico se mantienen 1:1. Solo edits editoriales: (1) `data/comparison-scenarios.ts` SCENARIO_WHY_10_FAILS — title "fracaso de PlanGrid" → "experiencia actual con PlanGrid", "trazabilidad falsa" → "sin trazabilidad individual", borrado del ejemplo "Pablo / Juanito" por descripción funcional, "Autodesk está cortando" → "Autodesk está aplicando restricciones progresivas", "Castillo en el aire" → "Mismo modelo, herramienta diferente". (2) `components/comparison/ComparativoNarrative.tsx` — "La pregunta obvia" → "La pregunta natural es", "rompió PlanGrid" → "limitó la experiencia con PlanGrid". (3) `components/proposal/VisionSection.tsx` — quote bloque "Esa es la idea... — Pablo Otero" reemplazado por tesis sin atribución. (4) `data/proposal.ts` GATE_CRITERIA g6 — "GO de Pablo" / "Pablo da GO" → "GO del sponsor" / "El sponsor da GO". (5) **PROPOSAL-v11.md** (NUEVO, derivado de v10) con todos los cambios de tono + "Héctor regresa de vacaciones" → "Reincorporación del gerente general", "Presentación a Héctor / dirección" → "Presentación a la dirección", "Para Pablo Otero Olivos · Gerente..." → "Para Grupo Inmobiliario Habita". `public/proposal.md` sincronizado. PDF/DOCX regenerados: `INFRATEK_..._v11.pdf` (383 KB) y `_v11.docx` (481 KB). **Site refs bumped v10 → v11**: las mismas 10+ ubicaciones que en el ciclo v9→v10. Render scripts defaults bumped a PROPOSAL-v11.md y `_v11.docx`. ⚠ El v11 PDF que Sergio mencionó en el prompt no existía físicamente; lo regeneré desde PROPOSAL-v11.md para evitar drift entre PDF y site. |
| Apr 30 | v11 production-ready. Shared with client. | Serge | **Final pass metódico antes de mandar a Pablo / Héctor.** (1) **PlanGrid corregido**: `data/proposal.ts` p3 impact + metric, `app/page.tsx` teaser, PROPOSAL-v11.md §1 + §13 — "1 cuenta para ~100 personas" → "una sola cuenta compartida por proyecto"; metric "1 de ~100 stakeholders" → "1 cuenta por proyecto". (2) **DeliverablesSection title** fix: redundante "Lo que Habita se queda con Habita" → "Lo que se queda con Habita". (3) **Em dashes purgados** de `data/`, `components/`, `app/`, `lib/` (todos los .tsx/.ts), `PROPOSAL-v11.md`, `public/proposal.md`, `public/proposal.html`, y el script `scripts/render-proposal.mjs` (template title + CSS comment). Reemplazos contextuales: section headers `—` → `:`, hito labels `—` → `:`, parentheticals `— X —` → `, X,`, separadores cortos `—` → `.` o `,`, `replaces: "—"` → `replaces: ""` (con SolutionSection check actualizado), null cost placeholder `"—"` → `"n/d"`. (4) **56% → +50% / "más de la mitad"**: `data/comparison-scenarios.ts` SCENARIO_DIAGNOSTIC stat, `data/proposal.ts` p2 impact. (5) **Ecosistema** suavizado: `data/autodesk-costs.ts` header comment + `components/comparison/AutodeskCosts.tsx` 3 strings. (6) **Final FIX 7 audits all clean**: zero matches en `.tsx/.ts` para Juanito, "— Pablo", Héctor, castillo en el aire, trazabilidad falsa, pregunta obvia, cortando cuentas, fracaso de PlanGrid, directorio, 5/cinco perfiles, mínimo 4/≥ 4 per/4 personas/4+ per, Cambios en v. PDF/DOCX regenerados: `INFRATEK_..._v11.pdf` (364 KB, ~20 KB menor por contenido limpio) + `_v11.docx` (480 KB). |
